...
If the server cannot reach `api.telegram.org:443`, Telegram needs server routing/VPN, a reverse proxy base URL in `TELEGRAM_API_BASE_URL`, or an HTTP(S) proxy in `TELEGRAM_PROXY_URL`. Telegram `tg://proxy?...` MTProto links are not supported by Bot API HTTP polling.
